...
void llc_tb::llc_test()
{
    ...
    switch (LLC_WAYS) {
        ...
        default: CACHE_REPORT_INFO("ERROR: LLC_WAYS not supported.");
    }
    ...
    // GetM, data. S -> M. l2#0.
    op(REQ_GETM, SHARED, 0, addr, null, 0, line_of_addr(addr.line), 0, MIN_L2 - 1, MIN_L2 - 1, 0,
       DATA);
    ...
    op_dma(DMA_READ, SHARED, EVICT, 0, addr1, addr2, 4, line_of_addr(addr1.line), 0, 1, 0, 0);
    ...
    op_dma(DMA_READ, MODIFIED, EVICT, DIRTY, addr1, addr2, 4, line_of_addr(addr1.line), 0, 1, 0, 0);
    ...
    op_dma(DMA_WRITE, SHARED, EVICT, 0, addr1, addr2, line_of_addr(addr1.line) * 2, 4, 0, 1, 0, 0);
    ...
    op_dma(DMA_WRITE, SHARED, EVICT, DIRTY, addr1, addr2, line_of_addr(addr1.line), 4,
           line_of_addr(addr2.line) * 2, 1, 0, 0);
    ...
    op_dma(DMA_WRITE, EXCLUSIVE, EVICT, 0, addr1, addr2, line_of_addr(addr1.line) * 2, 4, 0, 1, 0,
           0);
    ...
    op_dma(DMA_WRITE, EXCLUSIVE, EVICT, DIRTY, addr1, addr2, line_of_addr(addr1.line), 4,
           line_of_addr(addr2.line) * 2, 0, 0, 0);
    ...
    op_dma(DMA_WRITE, MODIFIED, EVICT, DIRTY, addr1, addr2, line_of_addr(addr1.line), 4,
           line_of_addr(addr2.line) * 2, 1, 0, 0);
    ...
    for (int i = 0; i < LLC_SETS; i++) {
        ...
        for (int j = 0; j < LLC_WAYS; j++) {
            ...
            if ((j % 4) == 0) {
                op(REQ_GETS, VALID, 0, addr, null, 0, line_of_addr(addr.line), 0, 0, 0, 0, DATA);
                op(REQ_GETS, EXCLUSIVE, 0, addr, null, 0, line_of_addr(addr.line), 0, 0, 1, 0,
                   DATA);
    ...
    for (int k = 0; k < wlength_size; k++) { // select burst length
        ...
        for (int i = 0; i < WORDS_PER_LINE; i++) {
            ...
            op_dma(DMA_READ, INVALID, 0, 0, addr, null, wlengths[k], line_of_addr(addr.line), 0, 0,
                   0, 0);
            ...
            op_dma(DMA_WRITE, VALID, 0, 0, addr, null, line_of_addr(addr.line), wlengths[k], 0, 0,
                   0, 0);
...
void llc_tb::op_rsp(coh_msg_t rsp_msg, addr_breakdown_llc_t req_addr, line_t req_line,
                    cache_id_t req_id)
{
...
void llc_tb::op(mix_msg_t coh_msg, llc_state_t state, bool evict, addr_breakdown_llc_t req_addr,
                addr_breakdown_llc_t evict_addr, line_t req_line, line_t rsp_line,
                line_t evict_line, invack_cnt_t invack_cnt, cache_id_t req_id, cache_id_t dest_id,
                hprot_t hprot)
{
    ...
    if ((state == INVALID || evict) && coh_msg != REQ_PUTS && coh_msg != REQ_PUTM) {
    ...
    switch (coh_msg) {
        ...
        case REQ_GETS:
            ...
            switch (state) {
                ...
                case VALID:
                    out_plane = RSP_PLANE;
                    if (hprot == INSTR) out_msg = RSP_DATA;
                    else // hprot ==  DATA
                        out_msg = RSP_EDATA;
                    break;
        ...
        default: CACHE_REPORT_INFO("ERROR: This request type is undefined.");
    }
...
void llc_tb::op_dma(mix_msg_t coh_msg, llc_state_t state, bool evict, bool dirty,
                    addr_breakdown_llc_t req_addr, addr_breakdown_llc_t evict_addr, line_t req_line,
                    line_t rsp_line, line_t evict_line, sharers_t sharers, owner_t owner,
                    bool stall)
{
    line_t wlength;
    bool done                 = false;
    ...
    if (coh_msg == REQ_DMA_READ_BURST) {
        word_t word = (req_line.range(ADDR_BITS - 1, 0).to_uint() * 4 + (BYTES_PER_WORD - 4)) /
            BYTES_PER_WORD;
        req_line                                                     = 0;
    ...
    for (int i = 0; i < llength; i++) {
        ...
        if (i == llength - 1) done = true;

        // req in
        if (coh_msg == REQ_DMA_WRITE_BURST || !i) {

            if (coh_msg == REQ_DMA_READ_BURST) wvalid = 0;
            ...
            put_dma_req_in(coh_msg, req_addr.line, req_line, 0, done, req_addr.w_off, wvalid);
        }
        ...
        if (llength == 1) invack_cnt.range(WORD_BITS, 1) = wlength - 1;
        ...
        if (coh_msg == DMA_READ)
            get_dma_rsp_out(RSP_DATA_DMA, req_addr.line, rsp_line, invack_cnt, 0, 0,
                            req_addr.w_off);
                         ...
                         cache_id_t req_id, cache_id_t dest_id, word_offset_t woff)
{
    ...
    if (rsp_out.coh_msg != coh_msg || rsp_out.addr != addr.range(TAG_RANGE_HI, SET_RANGE_LO) ||
        rsp_out.line != line || rsp_out.invack_cnt != invack_cnt || rsp_out.req_id != req_id ||
        rsp_out.dest_id != dest_id || rsp_out.word_offset != woff) {
    ...
    if (RPT_TB) CACHE_REPORT_VAR(sc_time_stamp(), "RSP_OUT", rsp_out);
}
...
void llc_tb::get_fwd_out(mix_msg_t coh_msg, addr_t addr, cache_id_t req_id, cache_id_t dest_id)
{
    ...
    if (fwd_out.coh_msg != coh_msg || fwd_out.addr != addr.range(TAG_RANGE_HI, SET_RANGE_LO) ||
        fwd_out.req_id != req_id || fwd_out.dest_id != dest_id) {
    ...
    if (RPT_TB) CACHE_REPORT_VAR(sc_time_stamp(), "FWD_OUT", fwd_out);
}
...
void llc_tb::get_mem_req(bool hwrite, addr_t addr, line_t line)
{
    ...
    if (mem_req.hwrite != hwrite || mem_req.addr != addr.range(TAG_RANGE_HI, SET_RANGE_LO) ||
        mem_req.line != line) {
    ...
    if (RPT_TB) CACHE_REPORT_VAR(sc_time_stamp(), "MEM_REQ", mem_req);
}
...
void llc_tb::put_mem_rsp(line_t line)
{
    ...
    if (RPT_TB) CACHE_REPORT_VAR(sc_time_stamp(), "MEM_RSP", mem_rsp);
}
                        ...
                        hprot_t hprot, word_offset_t woff, word_offset_t wvalid)
{
    ...
    if (RPT_TB) CACHE_REPORT_VAR(sc_time_stamp(), "REQ_IN", req_in);
}
...
void llc_tb::put_rsp_in(coh_msg_t rsp_msg, addr_t addr, line_t line, cache_id_t req_id)
{
    ...
    if (RPT_TB) CACHE_REPORT_VAR(sc_time_stamp(), "RSP_IN", rsp_in);
}
