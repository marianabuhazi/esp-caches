...
inline void llc::send_dma_rsp_out(coh_msg_t coh_msg, line_addr_t addr, line_t line,
...
void llc::ctrl()
{
    ...
    while (true) {
...

        {
            ...
            if (recall_pending) {
                if (!recall_valid) {
                    ...
                    if (can_get_rsp_in) { is_rsp_to_get = true; }
                }
                else {
            ...
            if (is_rst_to_get) { llc_rst_tb.nb_get(rst_in); }
        ...
        // Lookup cache
        // -----------------------------
        look = is_flush_to_resume || is_rsp_to_get || is_req_to_get || is_dma_req_to_get ||
        ...
        if (is_flush_to_resume) {
            ...
            for (int way = 0; way < LLC_WAYS; way++) {
                ...
                if (states_buf[way] == VALID && dirty_bits_buf[way]) {
                    ...
                    // std::cout << std::hex << "*** way: " << way << " set: " <<  set << " addr: "
                    // << new_addr_evict << " state: " << states_buf[way] << " line: " <<
                    // lines_buf[way] << std::endl;
                }
                else {
                    wait();
                }
        ...
        // Process response
            ...
            if (sharers_buf[way] != 0) states_buf[way] = SHARED;
            else
            ...
            switch (req_in.coh_msg) {
                    ...
                    switch (states_buf[way]) {
                            ...
                            if (req_in.hprot == 0) {
                                {
                                    ...
                                    send_rsp_out(RSP_DATA, req_in.addr, lines_buf[way],
                                                 req_in.req_id, 0, 0, 0);
                                }
                            ...
                            else {
                                {
                                    ...
                                    send_rsp_out(RSP_EDATA, req_in.addr, lines_buf[way],
                                                 req_in.req_id, 0, 0, 0);
                                }
                        ...
                        case SHARED: {
...

                            {
                                ...
                                send_rsp_out(RSP_DATA, req_in.addr, lines_buf[way], req_in.req_id,
                                             0, 0, 0);
                            }
                ...
                case REQ_PUTS:
...

                    {
                        ...
                        send_rsp_out(RSP_PUTACK, req_in.addr, 0, req_in.req_id, req_in.req_id, 0,
                                     0);
                    }
                    ...
                    switch (states_buf[way]) {
                        ...
                        case SHARED: {
                            ...
                            if (sharers_buf[way] == 0) states_buf[way] = VALID;
                        } break;

                        case EXCLUSIVE: {
                            PUTS_E;

                            if (owners_buf[way] == req_in.req_id) states_buf[way] = VALID;
                        } break;
        ...
        // Process DMA
            ...
            if (is_dma_req_to_get) {
                ...
                dma_read_length =
                    dma_req_in.line.range(BITS_PER_LINE - 1, BITS_PER_LINE - ADDR_BITS).to_uint();
                dma_length = 0;
                dma_done   = false;
                dma_start  = true;
            }
            ...
            if (!recall_pending || recall_valid) {
                ...
                if (is_dma_read_to_resume) {
                    ...
                    if (dma_start) dma_read_woffset = dma_req_in.word_offset;
                    else
                        dma_read_woffset = 0;

                    dma_length += WORDS_PER_LINE - dma_read_woffset;

                    if (dma_length >= dma_read_length) dma_done = true;

                    if (dma_start & dma_done) valid_words = dma_read_length;
                    else if (dma_start)
...

                    {
                        ...
                        send_dma_rsp_out(RSP_DATA_DMA, dma_addr, lines_buf[way], dma_req_in.req_id,
                                         0, dma_info, dma_read_woffset);
                    }
                    ...
                    if (misaligned) {
                        ...
                        for (int i = 0; i < WORDS_PER_LINE; i++) {
                            ...
                            if (word_cnt < valid_words && i >= dma_write_woffset) {
                                write_word(lines_buf[way], read_word(dma_req_in.line, i), i, 0,
                                           WORD);
                                word_cnt++;
                            }
                        }
                    ...
                    if (dma_req_in.hprot) { dma_done = true; }
                }
        ...
        // Resume flush
        ...
        else if (is_rsp_to_get || is_req_to_get || is_dma_req_to_get || is_dma_read_to_resume ||
            ...
            if (update_evict_ways) evict_ways.port1[0][set] = evict_ways_buf;

            // Uncomment for additional debug info during behavioral simulation
            // const line_addr_t new_addr_evict = (tags_buf[way] << LLC_SET_BITS) + set;
            // std::cout << std::hex << "*** way: " << way << " set: " <<  set << " addr: " <<
            // new_addr_evict << " state: " << states[llc_addr] << " line: " << lines[llc_addr] <<
            // std::endl;
        }
