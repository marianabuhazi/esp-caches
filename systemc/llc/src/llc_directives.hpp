...
#define LLC_MAP_MEMORY                                                                       \
    ...
    #define LLC_GETS bookmark_tmp |= BM_LLC_GETS

    #define LLC_GETM bookmark_tmp |= BM_LLC_GETM

    #define LLC_PUTS bookmark_tmp |= BM_LLC_PUTS

    #define LLC_PUTM bookmark_tmp |= BM_LLC_PUTM
    ...
    #define GENERIC_ASSERT asserts_tmp |= AS_GENERIC

    #define GETS_IV bookmark_tmp |= BM_GETS_IV

    #define GETS_S                                                   \
        bookmark_tmp |= BM_GETS_S;                                   \
        if (sharers_buf[way] == 0) asserts_tmp |= AS_GETS_S_NOSHARE; \
        if ((sharers_buf[way] & (1 << req_in.req_id)) != 0) asserts_tmp |= AS_GETS_S_ALREADYSHARE

    #define GETS_EM                 \
        bookmark_tmp |= BM_GETS_EM; \
        if (owner_buf[way] == req_in.req_id) asserts_tmp |= AS_GETS_EM_ALREADYOWN

    #define GETS_SD                 \
        bookmark_tmp |= BM_GETS_SD; \
        if ((sharers_buf[way] & (1 << req_in.req_id)) != 0) asserts_tmp |= AS_GETS_SD_ALREADYSHARE

    #define GETM_IV bookmark_tmp |= BM_GETM_IV

    #define GETM_S bookmark_tmp |= BM_GETM_S

    #define GETM_E                 \
        bookmark_tmp |= BM_GETM_E; \
        if (owner_buf[way] == req_in.req_id) asserts_tmp |= AS_GETM_EM_ALREADYOWN

    #define GETM_M                 \
        bookmark_tmp |= BM_GETM_M; \
        if (owner_buf[way] == req_in.req_id) asserts_tmp |= AS_GETM_EM_ALREADYOWN

    #define GETM_SD bookmark_tmp |= BM_GETM_SD

    #define PUTS_IVM bookmark_tmp |= BM_PUTS_IVM

    #define PUTS_S bookmark_tmp |= BM_PUTS_S

    #define PUTS_E bookmark_tmp |= BM_PUTS_E

    #define PUTS_SD bookmark_tmp |= BM_PUTS_SD

    #define PUTM_IV bookmark_tmp |= BM_PUTM_IV

    #define PUTM_S bookmark_tmp |= BM_PUTM_S

    #define PUTM_EM bookmark_tmp |= BM_PUTM_EM

    #define PUTM_SD bookmark_tmp |= BM_PUTM_SD

    #define DMA_READ_SD bookmark_tmp |= BM_DMA_READ_SD

    #define DMA_READ_I bookmark_tmp |= BM_DMA_READ_I

    #define DMA_READ_NOTSD bookmark_tmp |= BM_DMA_READ_NOTSD

    #define DMA_WRITE_SD bookmark_tmp |= BM_DMA_WRITE_SD

    #define DMA_WRITE_I bookmark_tmp |= BM_DMA_WRITE_I

    #define DMA_WRITE_NOTSD bookmark_tmp |= BM_DMA_WRITE_NOTSD
