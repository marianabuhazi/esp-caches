    ...
    while (true) {
        ...
        l2_way_t evict_way_tmp = 0;
        {
            ...
            if (l2_flush.nb_can_get() && reqs_cnt == N_REQS) {
                ...
                if (!fwd_stall) { get_fwd_in(fwd_in); }
                else {
                ...
                if (flush_set < L2_SETS) {
                    if (!l2_fwd_in.nb_can_get() && reqs_cnt != 0) do_ongoing_flush = true;
            ...
            else if ((l2_cpu_req.nb_can_get() || set_conflict) && !evict_stall &&
                     (reqs_cnt != 0 || ongoing_atomic)
#ifdef LLSC
                     && !ongoing_atomic_set_conflict_instr
#endif
            ) { // assuming HPROT cacheable
                if (!set_conflict) { get_cpu_req(cpu_req); }
                else {
            ...
            switch (rsp_in.coh_msg) {
                ...
                case RSP_EDATA: {
                    ...
                    if (ongoing_atomic_set_conflict_instr &&
                        ongoing_atomic_set_conflict_instr_line_addr == rsp_in.addr) {
                ...
                case RSP_DATA:
                    ...
                    switch (reqs[reqs_hit_i].state) {
                        ...
                        case ISD: {
                            ...
                            put_reqs(line_br.set, reqs[reqs_hit_i].way, line_br.tag, rsp_in.line,
                                     reqs[reqs_hit_i].hprot, SHARED, reqs_hit_i);
                        }
                        ...
                        case SMADW: {
                            ...
                            if (reqs[reqs_hit_i].invack_cnt == MAX_N_L2) {
                                ...
                                if (reqs[reqs_hit_i].amo == 0 && fwd_stall &&
                                    reqs_hit_i == reqs_fwd_stall_i) {
                ...
                case RSP_INVACK: {
                    ...
                    if (reqs[reqs_hit_i].invack_cnt == MAX_N_L2) {
                        ...
                        switch (reqs[reqs_hit_i].state) {
                            ...
                            case SMA: {
                                put_reqs(line_br.set, reqs[reqs_hit_i].way, line_br.tag,
                                         reqs[reqs_hit_i].line, reqs[reqs_hit_i].hprot, MODIFIED,
                                         reqs_hit_i);
                ...
                case RSP_PUTACK: {
                    ...
                    if (evict_stall) {
...
#ifdef LLSC
                        if (ongoing_atomic_set_conflict_instr &&
                            reqs[reqs_atomic_i].way == evict_way)
                            way_inc = 2;
#endif
                        ...
                        line_addr_t line_addr_tmp =
                            (reqs[reqs_hit_i].tag_estall << L2_SET_BITS) | (line_br.set);
                ...
                switch (reqs[reqs_hit_i].state) {
                    ...
                    case MIA: {
                        ...
                        if (fwd_in.coh_msg == FWD_GETS) {
                            ...
                            for (int i = 0; i < 2; i++) {
                                send_rsp_out(RSP_DATA, fwd_in.req_id, is_to_req[i], fwd_in.addr,
                                             reqs[reqs_hit_i].line);
                                wait();
                            }
...
// allow forwards between RISC-V LR/SC instructions
                    ...
                    case XMW: {
                        ...
                        if (fwd_in.coh_msg == FWD_GETS)
                            send_rsp_out(RSP_DATA, fwd_in.req_id, 0, fwd_in.addr,
                                         reqs[reqs_hit_i].line);
                        ...
                        if (fwd_in.coh_msg == FWD_GETS) {
                            put_reqs(reqs[reqs_hit_i].set, reqs[reqs_hit_i].way,
                                     reqs[reqs_hit_i].tag, reqs[reqs_hit_i].line,
                                     reqs[reqs_hit_i].hprot, SHARED, reqs_hit_i);
                        }
                        else {
                            send_inval(fwd_in.addr, reqs[reqs_hit_i].hprot);
                            states.port1[0][(reqs[reqs_hit_i].set << L2_WAY_BITS) +
                                            reqs[reqs_hit_i].way] = INVALID;
                        }
                        break;
                    }
#endif

                    default: FWD_HIT_DEFAULT;
                }
            }
            else {
                ...
                switch (fwd_in.coh_msg) {
                    ...
                    case FWD_GETM: {
                        ...
                        if (!ongoing_flush) send_inval(fwd_in.addr, hprot_buf[way_hit]);
...
#endif
            if ((state_buf[flush_way] != INVALID) && (is_flush_all || hprot_buf[flush_way])) {

                reqs_peek_flush(flush_set, reqs_hit_i);

                addr_t addr_tmp =
                    (tag_buf[flush_way] << L2_TAG_RANGE_LO) | (flush_set << SET_RANGE_LO);
                ...
                switch (state_buf[flush_way]) {
                    ...
                    default: EVICT_DEFAULT;
                }

                fill_reqs(0, addr_br, 0, flush_way, 0, state_tmp, 0, 0, line_buf[flush_way], 0,
                          reqs_hit_i);

                send_req_out(coh_msg_tmp, 0, line_addr_tmp, line_buf[flush_way]);
            }
            flush_way++;
        }
...
#endif
            {
                ...
                if (atomic_line_addr != addr_br.line_addr) {
                    ...
                    switch (cpu_req.cpu_msg) {
                        ...
                        case READ_ATOMIC: set_conflict = true; cpu_req_conflict = cpu_req;
#endif
                            reqs[reqs_atomic_i].state = INVALID;
                            reqs_cnt++;
                        ...
                        case WRITE_ATOMIC:
                            ...
                            if (cpu_req.cpu_msg == WRITE_ATOMIC) send_bresp(BRESP_EXOKAY);
                ...
                if (tag_hit) {
                    ...
                    switch (cpu_req.cpu_msg) {
                        ...
                        case READ: // read hit
                        {
                            ...
                            if (ongoing_atomic_set_conflict_instr) set_conflict = false;
                            ongoing_atomic_set_conflict_instr = false;
#endif
                            // read response
                        ...
                        case READ_ATOMIC: // read atomic hit
                            ...
                            switch (state_buf[way_hit]) {
                                ...
                                case SHARED: {
                                    ...
                                    fill_reqs(cpu_req.cpu_msg, addr_br, 0, way_hit, cpu_req.hsize,
                                              SMADW, cpu_req.hprot, cpu_req.word, line_buf[way_hit],
                                              cpu_req.amo, reqs_hit_i);

                                    // send request to directory
                                    send_req_out(REQ_GETM, cpu_req.hprot, addr_br.line_addr, 0);
                                } break;
                                ...
                                case MODIFIED:  // write hit
                                {
                                    ...
                                    // save request in intermediate state
                                    fill_reqs(cpu_req.cpu_msg, addr_br, 0, way_hit, cpu_req.hsize,
                                              XMW, cpu_req.hprot, cpu_req.word, line_buf[way_hit],
                                              cpu_req.amo, reqs_hit_i);
                                ...
                                default: HIT_READ_ATOMIC_DEFAULT;
                            }
                        ...
                        case WRITE:
                            ...
                            switch (state_buf[way_hit]) {
                                ...
                                case SHARED: {
                                    ...
                                    // save request in intermediate state
                                    fill_reqs(cpu_req.cpu_msg, addr_br, 0, way_hit, cpu_req.hsize,
                                              SMAD, cpu_req.hprot, cpu_req.word, line_buf[way_hit],
                                              cpu_req.amo, reqs_hit_i);
                                ...
                                case EXCLUSIVE: // write hit
                                    states.port1[0][(addr_br.set << L2_WAY_BITS) + way_hit] =
                                        MODIFIED;
                                case MODIFIED: // write hit
                                {
                                    HIT_WRITE_EM;
                                    ...
                                    write_word(line_buf[way_hit], cpu_req.word, addr_br.w_off,
                                               addr_br.b_off, cpu_req.hsize);
                                    lines.port1[0][(addr_br.set << L2_WAY_BITS) + way_hit] =
                                        line_buf[way_hit];
                                } break;

                                default: HIT_WRITE_DEFAULT;
                            }
                            break;
                    ...
                    switch (cpu_req.cpu_msg) {
                        ...
                        default: MISS_DEFAULT;
                    }

                    // save request in intermediate state
                    fill_reqs(cpu_req.cpu_msg, addr_br, 0, empty_way, cpu_req.hsize, state_tmp,
                              cpu_req.hprot, cpu_req.word, 0, cpu_req.amo, reqs_hit_i);
                    ...
                    line_addr_t line_addr_evict =
                        (tag_buf[evict_way_tmp] << L2_SET_BITS) | (addr_br.set);
                    ...
                    fill_reqs(cpu_req.cpu_msg, addr_br, tag_tmp, evict_way_tmp, cpu_req.hsize,
                              state_tmp, cpu_req.hprot, cpu_req.word, line_buf[evict_way_tmp],
                              cpu_req.amo, reqs_hit_i);
                }
            }
        }
...
void l2::send_rsp_out(coh_msg_t coh_msg, cache_id_t req_id, bool to_req, line_addr_t line_addr,
                      line_t line)
{
    SEND_RSP_OUT;
...
void l2::fill_reqs(cpu_msg_t cpu_msg, addr_breakdown_t addr_br, l2_tag_t tag_estall,
                   l2_way_t way_hit, hsize_t hsize, unstable_state_t state, hprot_t hprot,
                   word_t word, line_t line, amo_t amo, sc_uint<REQS_BITS> reqs_i)
{
    FILL_REQS;
...
void l2::put_reqs(l2_set_t set, l2_way_t way, l2_tag_t tag, line_t line, hprot_t hprot,
                  state_t state, sc_uint<REQS_BITS> reqs_i)
{
    PUT_REQS;
    ...
    if (fwd_stall && reqs_fwd_stall_i == reqs_i) { fwd_stall_ended = true; }
}
...
void l2::tag_lookup(addr_breakdown_t addr_br, bool &tag_hit, l2_way_t &way_hit,
                    bool &empty_way_found, l2_way_t &empty_way)
{
    TAG_LOOKUP;
...
void l2::tag_lookup_fwd(line_breakdown_t<l2_tag_t, l2_set_t> line_br, l2_way_t &way_hit)
{
    ...
    for (int i = L2_WAYS - 1; i >= 0; --i) {
        ...
        if (tag_buf[i] == line_br.tag && state_buf[i] != INVALID) { way_hit = i; }
    }
...
bool l2::reqs_peek_req(l2_set_t set, sc_uint<REQS_BITS> &reqs_i)
{
    ...
    for (unsigned int i = 0; i < N_REQS; ++i) {
        ...
        if (reqs[i].state == INVALID) reqs_i = i;

        if (reqs[i].set == set && reqs[i].state != INVALID) set_conflict = true;
    }
...
bool l2::reqs_peek_fwd(line_breakdown_t<l2_tag_t, l2_set_t> line_br, sc_uint<REQS_BITS> &reqs_i,
                       bool &reqs_hit, mix_msg_t coh_msg)
{
    ...
    for (unsigned int i = 0; i < N_REQS; ++i) {
        ...
        if (reqs[i].state != INVALID && reqs[i].tag == line_br.tag && reqs[i].set == line_br.set) {
            ...
            if (coh_msg == FWD_INV || coh_msg == FWD_INV_LLC) {
                if (reqs[i].state != ISD) fwd_stall_tmp = false;
            }
            else {
                if (reqs[i].state == MIA
#ifdef LLSC
