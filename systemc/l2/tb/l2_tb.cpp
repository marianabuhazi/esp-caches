...
void l2_tb::l2_test()
{
    ...
    for (int i = 0; i < L2_WAYS; ++i) {
        op(READ, MISS, 0, RSP_EDATA, 0, 0, WORD, addr, 0, line_of_addr(addr.line), 0, 0, 0, 0, 0,
           DATA);
        addr.tag_incr(1);
    }
    ...
    for (int i = 0; i < L2_WAYS; ++i) {
        op(READ, MISS, 0, RSP_DATA, 0, 0, WORD, addr, 0, line_of_addr(addr.line), 0, 0, 0, 0, 0,
           INSTR);
        addr.tag_incr(1);
    }
    ...
    // write misses
    for (int i = 0; i < L2_WAYS; ++i) {
        op(WRITE, MISS, 0, RSP_DATA, 0, 0, WORD, addr, word, line_of_addr(addr.line), 0, 0, 0, 0, 0,
           DATA);
    ...
    for (int wo = 0; wo < WORDS_PER_LINE; ++wo) {
        for (int wa = 0; wa < L2_WAYS; ++wa) {
            for (int s = 0; s < L2_SETS; ++s) {
                ...
                if (s >= L2_SETS / 2 && wo == 0) {
                    op(WRITE, MISS, 0, RSP_DATA, 0, 0, WORD, addr, word, line_of_addr(addr.line), 0,
                       0, 0, 0, 0, DATA);
                }
                else {
                    op(WRITE, HIT, 0, 0, 0, 0, WORD, addr, word, 0, 0, 0, 0, 0, 0, DATA);
        ...
        for (int s = 0; s < L2_SETS; ++s) {
            ...
            if (s < L2_SETS / 2) {
                for (wo = 0; wo < WORDS_PER_LINE / 2; ++wo) {
                    // TODO REMOVE write_word(line, addr.line + (wo << W_OFF_RANGE_LO) + 1, wo, 0,
                    // WORD);
                    write_word(line, word_of_addr(addr.line + (wo << W_OFF_RANGE_LO) + 1), wo, 0,
                               WORD);
                }
            }
    ...
    addr.tag_incr(1);
    op(READ, MISS, 0, RSP_EDATA, 0, 0, HALFWORD, addr, 0, line_of_addr(addr.line), 0, 0, 0, 0, 0,
       DATA);
    ...
    addr.tag_incr(1);
    op(WRITE, MISS, 0, RSP_DATA, 0, 0, HALFWORD, addr, 0, line_of_addr(addr.line), 0, 0, 0, 0, 0,
       DATA);
    ...
    for (int i = 0; i < L2_WAYS; ++i) {
        op(READ, MISS_EVICT, 0, RSP_EDATA, 0, REQ_PUTS, WORD, addr, 0, line_of_addr(addr.line), 0,
           0, 0, 0, 0, DATA);
    ...
    for (int i = 0; i < L2_WAYS; ++i) {
        op(READ, MISS_EVICT, 0, RSP_EDATA, 0, REQ_PUTM, WORD, addr, 0, line_of_addr(addr.line), 0,
           0, 0, 0, 0, DATA);
    ...
    for (int i = 0; i < n_reqs; i++) {
        ...
        put_cpu_req(cpu_req, READ, WORD, addr.word, 0, DATA);
        line = line_of_addr(addr.line);
        line.range(addr.w_off * BITS_PER_WORD + BITS_PER_WORD - 1, addr.w_off * BITS_PER_WORD) = 0;
        get_rd_rsp(line);
        wait();
    }
    ...
    do {
        ...
        addr5 = rand_addr();
             ...
             addr1.set == addr5.set || addr2.set == addr5.set || addr3.set == addr5.set ||
             addr4.set == addr5.set);
    ...
    for (int i = 0; i < L2_WAYS / 4; i++) {
        ...
        op(READ_ATOMIC, MISS, 0, RSP_DATA, 0, 0, WORD, addr, 0, line_of_addr(addr.line), 0, 0, 0, 0,
           0, DATA);
        // READ_ATOMIC(XMW)
        op(READ_ATOMIC, HIT, 0, 0, 0, 0, WORD, addr, 0, line_of_addr(addr.line), 0, 0, 0, 0, 0,
           DATA);
        // WRITE_ATOM(XMW)
    ...
    op(READ, MISS, 0, RSP_DATA, 0, 0, WORD, addr, 0, req_line, FWD_STALL, FWD_INV, 0, id--, 0,
       DATA);

    // FWD_GETM(IMAD), FWD_GETM(M)
    fwd_line = req_line;
    write_word(fwd_line, word, addr.w_off, addr.b_off, WORD);
    op(WRITE, MISS, 0, RSP_DATA, 0, 0, WORD, addr, word++, req_line, FWD_STALL, FWD_GETM, 0, id++,
       fwd_line, DATA);
    ...
    fwd_line = req_line;
    write_word(fwd_line, word, addr.w_off, addr.b_off, WORD);
    op(WRITE, MISS, 0, RSP_DATA, 0, 0, WORD, addr, word++, req_line, FWD_STALL, FWD_GETS, 0, id--,
       fwd_line, DATA);
    ...
    addr.tag_incr(1);
    req_line = line_of_addr(addr.line);
    op(WRITE, MISS, DATA_HALFWAY, RSP_DATA, MAX_N_L2 - 1, 0, WORD, addr, word++, req_line, 0, 0, 0,
       0, 0, DATA);
    ...
    addr.tag_incr(1);
    req_line = line_of_addr(addr.line);
    op(WRITE, MISS, 0, RSP_DATA, 0, 0, WORD, addr, word++, req_line, FWD_STALL, FWD_INV, SHARED,
       id++, 0, DATA);
    ...
    op(WRITE_ATOMIC, HIT, 0, 0, 0, 0, WORD, addr, word++, 0, FWD_STALL_XMW, FWD_GETS, 0, id--,
       fwd_line, DATA);
    ...
    fwd_line = req_line;
    write_word(fwd_line, word, addr.w_off, addr.b_off, WORD);
    op(READ_ATOMIC, MISS, DATA_FIRST, RSP_DATA, invack, 0, WORD, addr, 0, req_line, 0, 0, 0, 0, 0,
       DATA);
    op(WRITE_ATOMIC, HIT, 0, 0, 0, 0, WORD, addr, word++, 0, FWD_STALL_XMW, FWD_GETM, 0, id++,
       fwd_line, DATA);

    addr.tag_incr(1);
    req_line = line_of_addr(addr.line);
    op(READ_ATOMIC, MISS, DATA_HALFWAY, RSP_DATA, invack, 0, WORD, addr, 0, req_line, 0, 0, 0, 0, 0,
       DATA);

    addr.tag_incr(1);
    req_line = line_of_addr(addr.line);
    op(READ_ATOMIC, MISS, DATA_LAST, RSP_DATA, invack, 0, WORD, addr, 0, req_line, 0, 0, 0, 0, 0,
       DATA);
    ...
    addr.tag_incr(1);
    req_line = line_of_addr(addr.line);
    op(READ, HIT, 0, 0, 0, 0, WORD, addr, 0, req_line, FWD_NOSTALL, FWD_GETS, 0, id--, req_line,
       DATA);

    addr.tag_incr(1);
    req_line = line_of_addr(addr.line);
    op(READ, HIT, 0, 0, 0, 0, WORD, addr, 0, req_line, FWD_NOSTALL, FWD_GETM, 0, id++, req_line,
       DATA);
    ...
    flush(3 * L2_WAYS + 2 * (L2_WAYS - 1),
          flush_all); // 7 (addr1 set) + 8 (addr2 set) + 8 (addr3 set) + 7 (addr4 set)
    ...
    // fill a set
    for (int i = 0; i < L2_WAYS; ++i) {
        op(WRITE, MISS, 0, RSP_DATA, 0, 0, WORD, addr, word_of_addr(addr.word),
           line_of_addr(addr.line), 0, 0, 0, 0, 0, DATA);
    ...
    op(READ, MISS, 0, RSP_DATA, 0, 0, WORD, addr, 0, req_line, FWD_STALL, FWD_INV_LLC, 0, id--, 0,
       DATA);
    ...
    addr.tag_incr(1);
    req_line = line_of_addr(addr.line);
    op(WRITE, MISS, 0, RSP_DATA, 0, 0, WORD, addr, word++, req_line, FWD_STALL, FWD_INV_LLC, SHARED,
       id++, 0, DATA);
    ...
    op(READ_ATOMIC, MISS, DATA_FIRST, RSP_DATA, invack, 0, WORD, addr, 0, req_line, 0, 0, 0, 0, 0,
       DATA);
    op(WRITE_ATOMIC, HIT, 0, 0, 0, 0, WORD, addr, word++, 0, FWD_STALL_XMW, FWD_GETM_LLC, MODIFIED,
       id++, fwd_line, DATA);
    ...
    addr.tag_incr(1);
    req_line = line_of_addr(addr.line);
    op(READ, HIT, 0, 0, 0, 0, WORD, addr, 0, req_line, FWD_NOSTALL, FWD_GETM_LLC, EXCLUSIVE, id++,
       req_line, DATA);
    ...
    op(READ, MISS_EVICT, 0, RSP_DATA, 0, REQ_PUTM, WORD, addr, word_of_addr(addr.word),
       line_of_addr(addr.line), FWD_STALL_EVICT, FWD_GETS, 0, 1, line_of_addr(addr_evict.line),
       DATA);
    ...
    op(READ, MISS_EVICT, 0, RSP_DATA, 0, REQ_PUTM, WORD, addr, word_of_addr(addr.word),
       line_of_addr(addr.line), FWD_STALL_EVICT, FWD_GETM_LLC, MODIFIED, 0,
       line_of_addr(addr_evict.line), DATA);
...
void l2_tb::put_cpu_req(l2_cpu_req_t &cpu_req, cpu_msg_t cpu_msg, hsize_t hsize, addr_t addr,
                        word_t word, hprot_t hprot)
{
...
void l2_tb::get_rsp_out(coh_msg_t coh_msg, cache_id_t req_id, bool to_req, addr_t addr, line_t line)
{
...
void l2_tb::get_bresp(bresp_t gold_bresp)
{
    ...
    if (rpt) CACHE_REPORT_VAR(sc_time_stamp(), "BRESP", bresp);
}
...
void l2_tb::op(cpu_msg_t cpu_msg, int beh, int rsp_beh, coh_msg_t rsp_msg, invack_cnt_t invack_cnt,
               ...
               line_t rsp_line, int fwd_beh, mix_msg_t fwd_msg, state_t fwd_state,
               cache_id_t fwd_id, line_t fwd_line, hprot_t hprot)
{
    ...
    // else if (hsize == WORD)	cpu_addr.range(1, 0) = 0;
        ...
        cpu_addr.range(B_OFF_RANGE_HI - 1, 0) = 0;
    else if (hsize == WORD)
        cpu_addr.range(B_OFF_RANGE_HI, 0) = 0;
    else
        CACHE_REPORT_ERROR("Wrong hsize.", hsize);

    if (cpu_msg == READ) { coh_req = REQ_GETS; }
    ...
    if (cpu_msg == WRITE_ATOMIC) get_bresp(BRESP_EXOKAY);
    ...
    if (beh == MISS_EVICT) {
        ...
        if (fwd_beh == FWD_STALL_EVICT) {
            ...
            if (fwd_msg == FWD_INV) { get_rsp_out(RSP_INVACK, fwd_id, 1, req_addr_tmp.line, 0); }
                ...
                if (fwd_state == EXCLUSIVE) get_rsp_out(RSP_INVACK, 0, 0, req_addr_tmp.line, 0);
    ...
    if (cpu_msg == READ || cpu_msg == READ_ATOMIC) get_rd_rsp(rsp_line);
    ...
    if (fwd_beh != FWD_NONE && fwd_beh != FWD_STALL_EVICT) {
        ...
        if (fwd_msg == FWD_INV) {
            ...
            if (fwd_state == EXCLUSIVE) get_rsp_out(RSP_INVACK, 0, 0, req_addr.line, 0);
            else // MODIFIED
                get_rsp_out(RSP_DATA, 0, 0, req_addr.line, fwd_line);
        }
    }
