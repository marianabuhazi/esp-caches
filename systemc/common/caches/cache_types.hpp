...
class l2_cpu_req_t {
  ...
  public:
    ...
    l2_cpu_req_t() : cpu_msg(0), hsize(0), hprot(0), addr(0), word(0), amo(0) {}
    ...
    {
        return (x.cpu_msg == cpu_msg && x.hsize == hsize && x.hprot == hprot && x.addr == addr &&
                x.word == word && x.amo == amo);
    }
    ...
    {
        os << hex << "("
           << "cpu_msg: " << x.cpu_msg << ", hsize: " << x.hsize << ", hprot: " << x.hprot
           << ", addr: " << x.addr << ", word: " << x.word << ", amo: " << x.amo << ")";
        return os;
    }
};
...
class l2_rd_rsp_t {
  public:
    ...
    inline bool operator==(const l2_rd_rsp_t &x) const { return (x.line == line); }
...
class l2_fwd_in_t {
  ...
  public:
    ...
    l2_fwd_in_t() : coh_msg(0), addr(0), req_id(0) {}
    ...
    {
        return (x.coh_msg == coh_msg && x.addr == addr && x.req_id == req_id);
    }
    ...
    {
        os << hex << "("
           << "coh_msg: " << x.coh_msg << ", addr: " << x.addr << ", req_id: " << x.req_id << ")";
        return os;
    }
};
...
class l2_rsp_in_t {
  ...
  public:
    ...
    l2_rsp_in_t() : coh_msg(0), addr(0), line(0), invack_cnt(0) {}
    ...
    {
        return (x.coh_msg == coh_msg && x.addr == addr && x.line == line &&
    ...
    {
        os << hex << "("
           << "coh_msg: " << x.coh_msg << ", addr: " << x.addr << ", line: ";
        for (int i = WORDS_PER_LINE - 1; i >= 0; --i) {
            int base = i * BITS_PER_WORD;
            os << x.line.range(base + BITS_PER_WORD - 1, base) << " ";
        }
...
template <unsigned REQ_ID_WIDTH> class llc_rsp_out_t {
  ...
  public:
    ...
    llc_rsp_out_t() :
        coh_msg(0), addr(0), line(0), invack_cnt(0), req_id(0), dest_id(0), word_offset(0)
    {
    }
    ...
    {
        return (x.coh_msg == coh_msg && x.addr == addr && x.line == line &&
                x.invack_cnt == invack_cnt && x.req_id == req_id && x.dest_id == dest_id &&
    ...
    inline friend ostream &operator<<(ostream &os, const llc_rsp_out_t &x)
    {
        ...
        os << ", invack_cnt: " << x.invack_cnt << ", req_id: " << x.req_id
           << ", dest_id: " << x.dest_id << ", word_offset: " << x.word_offset << ")";
        return os;
    }
};
...
class llc_fwd_out_t {
  ...
  public:
    ...
    llc_fwd_out_t() : coh_msg(0), addr(0), req_id(0), dest_id(0) {}
    ...
    inline friend ostream &operator<<(ostream &os, const llc_fwd_out_t &x)
    {
        ...
        os << ", addr: " << x.addr << ", req_id: " << x.req_id << ", dest_id: " << x.dest_id << ")";
        return os;
    }
};
...
class l2_req_out_t {
  ...
  public:
    ...
    l2_req_out_t() : coh_msg(coh_msg_t(0)), hprot(0), addr(0), line(0) {}
    ...
    {
        return (x.coh_msg == coh_msg && x.hprot == hprot && x.addr == addr && x.line == line);
    }
    ...
    {
        os << hex << "("
           << "coh_msg: " << x.coh_msg << ", hprot: " << x.hprot << ", addr: " << x.addr
           << ", line: ";
        for (int i = WORDS_PER_LINE - 1; i >= 0; --i) {
            int base = i * BITS_PER_WORD;
            os << x.line.range(base + BITS_PER_WORD - 1, base) << " ";
...
template <unsigned REQ_ID_WIDTH> class llc_req_in_t {
  ...
  public:
    ...
    llc_req_in_t() :
        coh_msg(coh_msg_t(0)), hprot(0), addr(0), line(0), req_id(0), word_offset(0), valid_words(0)
    {
    }
    ...
    {
        return (x.coh_msg == coh_msg && x.hprot == hprot && x.addr == addr && x.line == line &&
                x.req_id == req_id && x.word_offset == word_offset && x.valid_words == valid_words);
    }
    ...
    inline friend ostream &operator<<(ostream &os, const llc_req_in_t &x)
    {
        ...
        os << ", hprot: " << x.hprot << ", addr: " << x.addr << ", req_id: " << x.req_id
           << ", word_offset: " << x.word_offset << ", valid_words: " << x.valid_words
           << ", line: ";
        for (int i = WORDS_PER_LINE - 1; i >= 0; --i) {
            int base = i * BITS_PER_WORD;
            os << x.line.range(base + BITS_PER_WORD - 1, base) << " ";
...
class l2_rsp_out_t {
  ...
  public:
    ...
    l2_rsp_out_t() : coh_msg(coh_msg_t(0)), req_id(0), to_req(0), addr(0), line(0) {}
    ...
    {
        return (x.coh_msg == coh_msg && x.req_id == req_id && x.to_req == to_req &&
                x.addr == addr && x.line == line);
    }
    ...
    {
        os << hex << "("
           << "coh_msg: " << x.coh_msg << ", req_id: " << x.req_id << ", to_req: " << x.to_req
           << ", addr: " << x.addr << ", line: ";
        for (int i = WORDS_PER_LINE - 1; i >= 0; --i) {
            int base = i * BITS_PER_WORD;
            os << x.line.range(base + BITS_PER_WORD - 1, base) << " ";
...
class llc_rsp_in_t {
  ...
  public:
    ...
    llc_rsp_in_t() : coh_msg(0), addr(0), line(0), req_id(0) {}
    ...
    {
        return (x.coh_msg == coh_msg && x.addr == addr && x.line == line && x.req_id == req_id);
    }
...
class llc_mem_req_t {
  ...
  public:
    ...
    llc_mem_req_t() : hwrite(0), hsize(0), hprot(0), addr(0), line(0) {}
    ...
    {
        return (x.hwrite == hwrite && x.hsize == hsize && x.hprot == hprot && x.addr == addr &&
                x.line == line);
    }
    ...
    {
        os << hex << "("
           << "hwrite: " << x.hwrite << ", hsize: " << x.hsize << ", hprot: " << x.hprot
           << ", addr: " << x.addr << ", line: " << x.line << ")";
        return os;
    }
};
...
class reqs_buf_t {
  ...
  public:
    ...
    reqs_buf_t() :
        cpu_msg(0), tag(0), tag_estall(0), set(0), way(0), hsize(0), w_off(0), b_off(0), state(0),
        hprot(0), invack_cnt(0), word(0), line(0), amo(0)
    {
    }
    ...
    {
        return (x.cpu_msg == cpu_msg && x.tag == tag && x.tag_estall == tag_estall &&
                x.set == set && x.way == way && x.hsize == hsize && x.w_off == w_off &&
                x.b_off == b_off && x.state == state && x.hprot == hprot &&
                x.invack_cnt == invack_cnt && x.word == word && x.line == line && x.amo == amo);
    }
    ...
    {
        os << hex << "("
           << "cpu_msg: " << x.cpu_msg << "tag: " << x.tag << "tag_estall: " << x.tag_estall
           << ", set: " << x.set << ", way: " << x.way << ", hsize: " << x.hsize
           << ", w_off: " << x.w_off << ", b_off: " << x.b_off << ", state: " << x.state
           << ", hprot: " << x.hprot << ", invack_cnt: " << x.invack_cnt << ", word: " << x.word
           << ", amo: " << x.amo << ", line: ";
        for (int i = WORDS_PER_LINE - 1; i >= 0; --i) {
            int base = i * BITS_PER_WORD;
            os << x.line.range(base + BITS_PER_WORD - 1, base) << " ";
...
class fwd_stall_backup_t {
  ...
  public:
    ...
    fwd_stall_backup_t() : coh_msg(0), addr(0) {}
...
class addr_breakdown_t {
  ...
  public:
    ...
    addr_breakdown_t() : line(0), line_addr(0), word(0), tag(0), set(0), w_off(0), b_off(0) {}
    ...
    {
        return (x.line == line && x.line_addr == line_addr && x.word == word && x.tag == tag &&
                x.set == set && x.w_off == w_off && x.b_off == b_off);
    }
    inline friend ostream &operator<<(ostream &os, const addr_breakdown_t &x)
    {
        os << hex << "("
           << "line: " << x.line << "line_addr: " << x.line_addr << ", word: " << x.word
           << ", tag: " << x.tag << ", set: " << x.set << ", w_off: " << x.w_off
           << ", b_off: " << x.b_off << ")";
        return os;
    }
...
class addr_breakdown_llc_t {
  ...
  public:
    ...
    addr_breakdown_llc_t() : line(0), line_addr(0), word(0), tag(0), set(0), w_off(0), b_off(0) {}
...
// line breakdown
template <class tag_t, class set_t> class line_breakdown_t {

  public:
    tag_t tag;
    set_t set;

    line_breakdown_t() : tag(0), set(0) {}
    ...
    void tag_incr(int a) { tag += a; }

    void set_incr(int a) { set += a; }

    void tag_decr(int a) { tag -= a; }

    void set_decr(int a) { set -= a; }
